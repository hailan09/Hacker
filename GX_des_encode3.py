...
def result():
    while True:
        ...
        try:
            ...
            md5_hash.update(input_plaintext.encode('utf-8'))

            # 获取哈希值的十六进制表示
            md5_input_plaintext = md5_hash.hexdigest()

            print(f"md5_input_plaintext:{md5_input_plaintext}")

            md5_input_plaintext_1 = md5_input_plaintext[:13]

            md5_input_plaintext_2 = md5_input_plaintext[-13:]

            two_string = ""  # 特征字符2

            two_passwd = bytes(two_string + ciphertext12 + md5_input_plaintext_1 + md5_input_plaintext_2 + ciphertext_12,
                               'utf-8')  # 特征码+第一重密文的后12位数转换为bytes类型的明文，第二重密钥由第一重前后各12位密文+第一重密钥构成,转为bytes类型

            print(f"two_passwd:{two_passwd}")
            ...
            print(f"tow_ciphertext:{tow_ciphertext}")

            md5_hash.update(tow_ciphertext.encode('utf-8'))

            md5_tow_ciphertext = md5_hash.hexdigest()

            print(f"md5_tow_ciphertext:{md5_tow_ciphertext}")

            # 提取中间8位字符串
            middle_8 = md5_tow_ciphertext[12:20]

            # 前12位字符串
            first_12 = md5_tow_ciphertext[:12]

            # 提取前12位字符串中的3位数
            part1 = first_12[3] + first_12[7] + first_12[11]

            # 后12位字符串
            last_12 = md5_tow_ciphertext[-12:]

            # 提取后12位字符串中的4位数
            part2 = last_12[0] + last_12[3] + last_12[6] + last_12[9]

            # 拼接结果
            result_tow_ciphertext = part2 + middle_8 + part1


            my_password = "G" + result_tow_ciphertext

            # 复制文本到剪贴板
            pyperclip.copy(my_password)

            print("输出：" + my_password)
...
if __name__ == '__main__':
    ...
    # input_plaintext = input("请输入明文：")
    # get_pwd = getpass.getpass("请输入密钥：")
    # result(input_plaintext,get_pwd)
